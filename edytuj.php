        ...
        <div class="row">
            <div class="col-md-12"><a href="/socket" class="btn btn-success" style="margin-top:20px">&larr; Powrót do strony głównej</a><br/>
                ...
                <div id="content">
                    ...
                    $ret = '<form action="editact.php" method="post"><table class="table table-bordered">';
                    foreach($tbHead as $k => $t) {
                        if($t['Field']!='id') { 
                            $ret.= '<tr><th>'.$t['Field'].'</th><td><input type="text" class="form-control" name="tb_'.$t['Field'].'" value="'.$row[$t['Field']].'"></td></tr>';
                        }
                        else {
                            $ret.= '<tr><th>'.$t['Field'].'</th><td><input type="text" class="form-control" value="'.$row[$t['Field']].'" disabled="disabled"></td></tr>';                            
                        }
                    }
                    $ret.='</table><input type="hidden" name="tab" value="'.$tab.'"><input type="hidden" name="id" value="'.$id.'"><input type="submit" value="Zapisz zmiany" class="btn btn-danger btn-block"></form>';
                    echo $ret;
                    ?>
                </div>
            </div>
